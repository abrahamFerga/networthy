# Builds the Networthy web UI from the Cortex frontend packages and embeds it into the host.
#
...
# same origin as the API, no CORS, no registry. This script bakes Networthy's branding and a
# same-origin API base into the bundles, then copies them into src/Networthy.Host/wwwroot.
...
$env:VITE_API_BASE = ""
try {
    Write-Host "Building @cortex/ui (branded 'Networthy', same-origin API)..." -ForegroundColor Cyan
